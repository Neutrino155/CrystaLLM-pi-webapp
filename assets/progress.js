...
function startProgress() {
    ...
    let modelSizeText = document.querySelector("#model-size .Select-value-label").innerText;
    let isLarge = modelSizeText.startsWith("small") || modelSizeText.startsWith("large");

    let hasZ = !!document.querySelector("#formula-unit .Select-value-label");

    let phase1Percent = (hasZ) ? 80 : 60;
    
    let phase1Upper = (hasZ) ? 3.4 : 2;
    let phase1Lower = 1;
    let phase2Upper = (hasZ) ? 0.4 : 0.6;
    let phase2Lower = (hasZ) ? 0.2 : 0.3;
    if (isLarge) {
        phase1Upper = (hasZ) ? 1.8 : 1;
        phase1Lower = 0.5;
